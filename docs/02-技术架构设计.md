...
│   │   ├── dream.ts        # 梦境相关接口
│   │   ├── analysis.ts     # 解析相关接口
│   │   ├── polish.ts       # AI润色相关接口 (新增)
│   │   ├── explore.ts      # 探索相关接口 (新增)
│   │   ├── task.ts         # 任务相关接口
│   │   └── ad.ts           # 广告相关接口
│   ├── request.ts          # 请求封装
│   └── index.ts            # 统一导出
...
│   │   ├── VersionSwitcher.vue # 版本切换器 (新增)
│   │   ├── PolishButton.vue    # AI润色按钮 (新增)
│   │   ├── PrivacyToggle.vue   # 隐私开关 (新增)
│   │   └── ExploreDreamCard.vue # 公开梦境卡片 (新增)
│   └── layout/             # 布局组件
│       ├── NavBar.vue      # 导航栏
│       └── TabBar.vue      # 底部栏
│
...
│   ├── useUser.ts          # 用户相关
│   ├── useDream.ts         # 梦境相关
│   ├── usePolish.ts        # AI润色相关 (新增)
│   ├── useExplore.ts       # 探索相关 (新增)
│   ├── useVersion.ts       # 版本管理相关 (新增)
│   ├── useAd.ts            # 广告相关
│   ├── useAuth.ts          # 认证相关
│   └── useStorage.ts       # 存储相关
│
...
│   ├── dream.d.ts
│   ├── analysis.d.ts
│   ├── polish.d.ts         # AI润色类型 (新增)
│   ├── explore.d.ts        # 探索类型 (新增)
│   ├── version.d.ts        # 版本类型 (新增)
│   ├── task.d.ts
│   └── api.d.ts
│
├── utils/                  # 工具函数
...
export function useDream() {
    ...
    const todayDream = computed(() => {
        ...
        return dreamList.value.find((d) => new Date(d.createdAt).toDateString() === today);
    });
...
user:{userId}:polish_quota         # AI润色配额 (新增)

# 梦境相关
dream:{dreamId}:detail
dream:{dreamId}:current_version    # 当前查看版本 (新增)
dream:{dreamId}:versions           # 版本列表 (新增)
dream:user:{userId}:list
dream:user:{userId}:calendar:{month}

# 公开梦境 (新增)
explore:public_dreams:list         # 公开梦境列表
explore:public_dreams:random       # 随机探索池
explore:user:{userId}:viewed       # 用户已查看的公开梦境
...
rate_limit:user:{userId}:polish       # AI润色限流 (新增)
rate_limit:user:{userId}:explore      # 探索限流 (新增)
